...
func GetMod2(b []byte) (ConditionListInfo, error) {
	...
	dayLength, deleteLength, activeLength := getLength()

	// expire, err := strconv.ParseInt(data[0], 10, 64)
	expire, err := strconv.ParseInt(s[:dayLength], 10, 64)
	if err != nil {
		return ConditionListInfo{}, err
	}

	// deleteFlag := data[1] == "1"
	deleteFlag := s[dayLength:dayLength+deleteLength] == "1"

	// activeFlag := data[2] != "0"
	activeFlag := s[dayLength+deleteLength:dayLength+deleteLength+activeLength] != "0"

	// ioids := strings.Split(data[3], CtrlB)
	// ioidsStr := data[3]
	ioidsStr := s[dayLength+deleteLength+activeLength:]
	length := len(ioidsStr)
...
var conditionListIoidsPool2 = sync.Pool{
	New: func() interface{} {
		s := make([]string, getIoidNum())
		return &s
	},
}

func GetMod2SyncPool(b []byte) (ConditionListInfo, error) {
	s := *(*string)(unsafe.Pointer(&b))
	...
	ioidsPtr := conditionListIoidsPool2.Get().(*[]string)
	ioids := *ioidsPtr
	fmt.Println("len(ioids),cap(ioids)", len(ioids), cap(ioids))
	// ioids = ioids[:0]
	fmt.Println("after len(ioids),cap(ioids)", len(ioids), cap(ioids))
	...
	*ioidsPtr = ioids
	conditionListIoidsPool2.Put(ioidsPtr)
...
var (
	mu               sync.RWMutex
	DayLength        int
	DeleteFlagLength int
	ActiveFlagLength int

	IoidNum int
)

func setLength(day, delete, active int) {
	mu.Lock()
	DayLength = day
	DeleteFlagLength = delete
	ActiveFlagLength = active
	mu.Unlock()
}

func getLength() (int, int, int) {
	mu.Lock()
	defer mu.Unlock()
	return DayLength, DeleteFlagLength, ActiveFlagLength
}
...
func dummyConditionListMod2(n int) []byte {
	...
	daystr := strconv.FormatInt(span*int64(day), 10)

	defer func() {
		setLength(len(daystr), len(deleteFlag), len(activeFlag))
	}()
	res := daystr + deleteFlag + activeFlag + ioids
	return []byte(res)
}

func setIoidNum(ioidNum int) {
	mu.Lock()
	IoidNum = ioidNum
	mu.Unlock()
}
...
var DummyConditionListMod3 = dummyConditionListMod3(100)
...
func TestConditionListInfoDictionaryGet(t *testing.T) {
	...
	for i := 0; i < num; i++ {
		...
		go func() {
			...
			dummyMod3 := dummyConditionListMod3(50 + i)
			...
			var infos5 ConditionListInfo
			fmt.Println("AllocsPerRun5:", int(testing.AllocsPerRun(100, func() {
				infos5, err = GetMod2SyncPool(dummyMod3)
				if err != nil {
					log.Fatal(err)
				}
			})))
			if diff := cmp.Diff(infos5, infos); diff != "" {
				t.Errorf("(-infos5 +infos)\n%s", diff)
			}
		}()
	}
	wg.Wait()
